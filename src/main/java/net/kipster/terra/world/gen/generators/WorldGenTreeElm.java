...
import net.kipster.terra.objects.blocks.BlockLeavesElm;
import net.kipster.terra.objects.blocks.BlockLogCherry;
import net.kipster.terra.objects.blocks.BlockLogElm;
...
public class WorldGenTreeElm extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    List<WorldGenTreeElm.FoliageCoordinates> foliageCoords;

    public WorldGenTreeElm(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenTreeElm.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenTreeElm.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenTreeElm.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crosSection(pos.up(i), this.leafSize(i), BlockInit.ELMLEAVES.getDefaultState().withProperty(BlockLeavesElm.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            this.setBlockAndNotifyAdequately(this.world, blockpos1, p_175937_3_.getDefaultState().withProperty(BlockLogElm.LOG_AXIS, blocklog$enumaxis));
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenTreeElm.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
