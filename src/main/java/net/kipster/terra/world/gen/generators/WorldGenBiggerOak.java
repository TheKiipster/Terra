...
public class WorldGenBiggerOak extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    List<WorldGenBiggerOak.FoliageCoordinates> foliageCoords;

    public WorldGenBiggerOak(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        {
            this.height = this.heightLimit - 8;
        }

        int i = (int)(1.382D + Math.pow(this.leafDensity * (double)this.heightLimit /20.0D, 8.0D));
        ...
        this.foliageCoords = Lists.<WorldGenBiggerOak.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenBiggerOak.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenBiggerOak.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeaves()
    {
        for (WorldGenBiggerOak.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        {
            this.heightLimit = 10 + this.rand.nextInt(this.heightLimitLimit);
        }
